    ...
    <main className="min-h-screen bg-[var(--ds-bg)] text-[var(--ds-text)] font-body">
      ...
      >
        <div className="mx-auto max-w-[1320px] px-6 py-12 md:px-10 md:py-16">
          ...
          <h1 className="mt-3 max-w-[28ch] font-display-serif text-3xl font-bold leading-[1.1] tracking-tight text-white md:text-5xl">
            Seven agents.<br />
            <span className="italic text-[var(--ds-warn)]">Working on a clock.</span>
          </h1>
          ...
          <div className="grid gap-px bg-tx-ink/10 md:grid-cols-2 lg:grid-cols-3">
            {ROSTER.map((a) => (
              <Link
                key={a.id}
                href={a.detail_path}
                className="group relative block bg-white p-6 transition-colors hover:bg-[var(--ds-bg)]"
              >
                <span
                  aria-hidden
                ...
                <div className="ml-3">
                  ...
                  <h2 className="mt-2 font-display-serif text-2xl font-bold text-[var(--ds-text)] group-hover:text-[var(--ds-warm)]">
                    {a.label}
                  </h2>
                  <p className="mt-3 text-sm leading-relaxed text-[var(--ds-text)]">{a.description}</p>
                  <div className="mt-5 border-t border-[var(--ds-border)] pt-3">
                    <p className="font-mono text-[10px] uppercase tracking-wider text-[var(--ds-text)]/55">Schedule</p>
                    <p className="mt-1 font-mono text-[12px] text-[var(--ds-text)]">{a.schedule}</p>
                  </div>
                  <p className="mt-4 font-mono text-[11px] font-semibold uppercase tracking-wider text-[var(--ds-warm)] opacity-0 group-hover:opacity-100">
                    See activity →
                  </p>
                </div>
              </Link>
              ...
              <ul className="divide-y divide-tx-ink/10">
                {recentRuns.map((r) => (
                  <li key={r.hash} className="flex items-baseline gap-4 px-6 py-4">
                    <p className="font-mono text-[10px] tabular-nums text-[var(--ds-text)]/55 min-w-[80px]">
                      {new Date(r.savedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </p>
                    <div className="min-w-0 flex-1">
                      <p className="font-display-serif text-sm font-semibold text-[var(--ds-text)]">
      ...
      <footer className="bg-[var(--ds-bg-deep)] text-white">
        <div className="mx-auto max-w-[1320px] px-6 py-6 md:px-10">
          <p className="text-[13px] text-white/60">
            ...
            <Link href="/" className="text-white/85 hover:text-[var(--ds-warn)]">← Back to TXLookup</Link>
          </p>
        </div>
      </footer>
    </main>
...
function StatusDot({ status }: { status: "live" | "scheduled" | "on-demand" }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="font-mono text-[10px] uppercase tracking-wider text-[var(--ds-text)]/65">{config.label}</span>
    </div>
  );
}
