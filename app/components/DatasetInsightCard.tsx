...
// brand-faithful per BRAND.md (tx-navy / tx-rust / tx-gold / tx-cream tokens).
//
    ...
    <section className="border-b border-tx-ink/10 bg-tx-cream">
      <div className="mx-auto max-w-[1320px] px-6 py-12 md:px-10 md:py-16">
        ...
        >
          ...
          <div>
              ...
              {chips.map((q, i) => (
                <li key={i}>
                  <Link
                    ...
                    className="group flex items-center justify-between gap-4 rounded-[10px] border border-tx-ink/10 bg-white px-5 py-4 transition hover:border-tx-rust/40 hover:bg-tx-gold-light/40"
                  >
                    <span className="font-display text-[15px] font-medium leading-snug text-tx-navy md:text-base">
                      {q}
                    </span>
                    <span
                      aria-hidden
                      className="font-mono text-sm text-tx-rust transition group-hover:translate-x-0.5"
                    >
                      ask →
                    </span>
                  </Link>
            ...
            {more.length > 0 && (
              <details className="mt-5 group">
                <summary className="cursor-pointer list-none font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-tx-sky hover:text-tx-rust">
                  ...
                  {more.map((q, i) => (
                    <li key={i}>
                      <Link
                        ...
                        className="block rounded-sm border-l-2 border-tx-gold/60 bg-white/60 px-4 py-2 text-sm text-tx-ink/85 hover:border-tx-rust hover:bg-tx-gold-light/40 hover:text-tx-navy"
                      >
                        {q}
                      </Link>
                    </li>
          ...
          {hasInsight && (
            <aside
              className="flex flex-col rounded-[10px] border-l-[3px] border-tx-rust bg-tx-navy p-6 text-tx-cream"
            ...
            >
              ...
              <p className="mt-3 font-display text-[44px] font-normal leading-none tracking-tight text-tx-cream md:text-[52px]">
                {liveInsight!.value}
              </p>
              <p className="mt-3 text-sm leading-snug text-tx-cream/80">
                {liveInsight!.label}
              </p>
              {liveInsight!.delta && (
                <p className="mt-4 inline-block w-fit rounded-sm bg-tx-gold-light/15 px-2 py-1 font-mono text-[11px] font-semibold uppercase tracking-[0.1em] text-tx-gold">
                  {liveInsight!.delta}
                </p>
              )}
            </aside>
        ...
        {/* ── Report CTA ── */}
        <div className="mt-8 border-t border-tx-ink/10 pt-5 text-sm">
          {reportSlug ? (
            <Link
            ...
            <a
              ...
              className="font-display font-semibold text-tx-sky hover:text-tx-rust hover:underline"
            >
