...
| `SOCRATA_KEY_ID` + `SOCRATA_KEY_SECRET` | Higher rate limits on open data calls (HTTP Basic auth — recommended). Used by both `app/lib/socrata.ts` and `agent/tools/data.py`. | Sign up at https://evergreen.data.socrata.com/signup, then **Profile → Edit Profile → Developer Settings** → generate API key. Free. Cross-portal. |
...
**Socrata returns 403 / 429** → sign up at https://evergreen.data.socrata.com/signup, go to **Profile → Edit Profile → Developer Settings**, generate an API key, and set `SOCRATA_KEY_ID` + `SOCRATA_KEY_SECRET`. Docs: https://dev.socrata.com/docs/app-tokens.html.
